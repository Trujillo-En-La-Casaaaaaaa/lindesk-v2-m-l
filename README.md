# ShopFlow — migración · complejidad baja (M-L)

Paquete de entrevista (artefacto congelado). Contiene el árbol de ShopFlow del escenario **M-L**. **No es LinDesk**; es el software de dominio que se evalúa.
...
| Complejidad arquitectónica | Baja — de legado a MVC en un repositorio |
| Identificador de corrida | `20260917-080537` (solo trazabilidad) |

Baseline **antes** de LinDesk (AS-IS): https://github.com/Trujillo-En-La-Casaaaaaaa/lindesk-v2-m-l-as-is
...
Partir del ShopFlow legado (un monolito de script de transacción) y dejarlo en **MVC** limpio: un repositorio, una app, separación tipo controladores / servicios / repositorios / modelos / vistas, PostgreSQL. Sin microservicios ni repositorios extra.

### Encargo (resumen)

Reestructurar la arquitectura **preservando** el comportamiento observable (catálogo, stock, pedidos, envío, notificación). **No añadir cancelación de pedido.** El objetivo es migración arquitectónica, no una feature nueva. Incluir o mantener pruebas que demuestren preservación.

El texto **exacto** del encargo está en [`TASK.md`](./TASK.md). Úselo como contrato.

### Carpetas de este árbol

- `shopflow-app/` — resultado migrado (un repositorio).
